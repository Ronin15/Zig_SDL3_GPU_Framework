...
description: >
  Zig game engine code review specialist. Use when asked to review Zig changes,
  pull requests, diffs, refactors, tests, rendering code, SDL3/SDL_GPU integration,
  fixed-step game loops, state stacks, input routing, asset handling, resource
  lifetimes, or performance-sensitive game-engine code. Also use when running
  /zig-review-specialist.
when-to-use: >
  Review code, PRs, or diffs. Audit Zig changes for correctness, ownership
  boundaries, resource lifetimes, SDL_GPU usage, performance risks, ECS shape,
  threading/cache-line issues, test coverage, or diagnostics. Perform code review
  on implementation work, refactors, or new features in this engine.
metadata:
